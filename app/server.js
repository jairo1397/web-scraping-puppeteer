...
var siguiente = [];
async function buscar_ofertas(buscar) {
    // locanto
    ...
    // }
    // siguiente.push(text.siguiente);
    return { ofertas_laborales, siguiente }
}


// ofertas laborales enviar una palabra por el url
app.get('/ofertas_laborales/:palabra', function (req, res) {
    // si la palabra tiene %20
    if (req.params.palabra.includes("%20")) {
        // reemplazar por espacio
        req.params.palabra = req.params.palabra.replace("%20", " ");
    }
    // buscar ofertas
    buscar_ofertas(req.params.palabra).then((data) => {
        res.json(data);
    })
})
